...
final class TrackerWhitelistClient
{
    ...
    /** Validated base URL (scheme http/https, no private/reserved host) or null. */
    ...
    public static function validateUrl(string $url): ?string
    {
        ...
        $host = $parts['host'];
        // Admin-trusted setting; still refuse loopback/private targets unless the host is an IP the
        // admin typed on purpose (a local tracker on the same box IS a valid setup, e.g. 127.0.0.1).
        if (!filter_var($host, FILTER_VALIDATE_IP)) {
            $ips = @gethostbynamel($host) ?: [];
            foreach ($ips as $ip) {
                if (!filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_NO_RES_RANGE)) return null;
            }
        }
        return rtrim($url, '/');
    }
    ...
    private function request(string $method, string $endpoint, ?array $body, int $timeout, ?array $override = null): array
    {
        ...
        if ($keyId === '' || $secret === '') return ['ok' => false, 'status' => 0, 'json' => null, 'error' => 'missing_credentials', 'elapsed_ms' => 0];
